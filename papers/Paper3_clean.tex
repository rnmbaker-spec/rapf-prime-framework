...
Complementing the theoretical lattice structure, we present an empirical equidistribution census for prime triplets across triplet-admissible residue classes modulo $P_n$. Building on the sub-Poisson variance suppression observed in twin primes, the triplet census reveals an even more pronounced structural anti-clumping effect, with the Coefficient of Variation strictly below the Poisson baseline. This work constitutes Paper 3 in the Recursive Admissibility Prime Framework (RAPF) trilogy, demonstrating that as $k$-tuple constellation density thins, the rigid arithmetic spacing of the lattice exerts an increasingly dominant suppressive force on local variance.
\end{abstract}
...
This paper extends the discrete information-geometric framework to the modulo-30 lattice ($\mathcal{L}_{30}$) and presents the corresponding equidistribution census. We demonstrate that the variance suppression observed for twin primes (Paper 2) is a generalized feature of admissible classes, becoming structurally amplified in $k=3$ constellations.
...
As $\lambda \to \infty$, the small-angle approximation $\sinh(15/\lambda) \approx 15/\lambda$ yields $I(\lambda) \to 225/[\lambda^4 (225/\lambda^2)] = 1/\lambda^2$. Thus, the discrete modulo-30 geometry flawlessly recovers the continuous exponential information limit. However, at computable scales, the $\sinh$ formulation is geometrically required to accurately measure information bounded by $P_3$ constraints.
\end{remark}
...
\toprule
$n$ & $P_n$ & $A_{n,3}$ & Total Triplets & Mean ($\mu$) & CV & CV$_{\rm Poisson}$ & Empty \\
\midrule
3 & 30 & 4 & 25 & 6.3 & 23.66\% & 40.00\% & 0 \\
4 & 210 & 16 & 284 & 17.8 & 12.83\% & 23.74\% & 0 \\
5 & 2{,}310 & 128 & 10{,}381 & 81.1 & 9.50\% & 11.10\% & 0 \\
6 & 30{,}030 & 1{,}280 & 695{,}318 & 543.2 & 3.98\% & 4.29\% & 0 \\
\textbf{7} & \textbf{510{,}510} & \textbf{17{,}920} & \textbf{93{,}169{,}155} & \textbf{5{,}199.2} & \textbf{1.30\%} & \textbf{1.39\%} & \textbf{0} \\
\bottomrule
\end{tabular}

\vspace{0.5em}
\noindent{\footnotesize CV$_{\rm Poisson} = 100/\sqrt{\mu}$ represents the independent Poisson baseline. Combined classes = $2 \times A_{n,3}$ (both patterns). Total triplets combines both $\mathcal{H}_1 = (0, 2, 6)$ and $\mathcal{H}_2 = (0, 4, 6)$. Ranges: $n=3$ covers $[30, 900]$, $n=4$ covers $[210, 44{,}100]$, $n=5$ covers $[2{,}310, 5{,}336{,}100]$, $n=6$ covers $[30{,}030, 9.02 \times 10^8]$, $n=7$ covers $[510{,}510, 2.61 \times 10^{11}]$.
}
\end{table}
...
The $n=6 \rightarrow n=7$ transition is particularly significant. The CV ratio drops from 3.98\% to 1.30\%---a factor of $0.33$---the largest single-step reduction in the trajectory. This corresponds to a heuristic information scaling ratio of $9.40\times$ in Fisher information under the proxy $I \propto 1/\text{CV}^2$ in a single primorial jump. The previously observed plateau between $n=5$ and $n=6$ (CV ratio $0.95\times$) was a localized scale-range artifact; the $n=7$ data demonstrates that the $1/\lambda^2$ fixed-point flow has fully asserted its dominance over local arithmetic noise.

\begin{remark}[Variance--Metric Scaling]
The stated $9.40\times$ scaling ratio is a heuristic proxy derived by treating the inverse observed dispersion as proportional to the score variance ($I \propto 1/\text{CV}^2$). While this provides a consistent comparative measure of lattice tightening across primorial scales, it is an empirical benchmark rather than a formal closed-form prediction of the geometric PMF model. The model's Fisher information $I(\lambda)$ governs the expected score variance of the geometric distribution; the mapping from empirical class-count CV to $I(\lambda)$ assumes that the class-count variance is driven primarily by fluctuations in the underlying gap distribution parameters.
\end{remark}
...
The mirror symmetry between Pattern A and Pattern B is a notable structural feature. Over 46 million triplet pairs per pattern, the two chiral configurations exhibit nearly identical dispersion (CV difference of $0.02\%$), identical class occupancy (zero empties), and total counts that differ by only $0.03\%$. This statistical equilibrium confirms that the variance suppression mechanism acts uniformly on both orientations of the triplet lattice---the rigid geometry of the primorial scaffolding, not any idiosyncratic arithmetic preference, is the governing structure.
...
\begin{itemize}
  ...
  \item $p \equiv 1 \pmod 3$ (mod 3: 1 choice, since $p+2 \equiv 0$ and $p+8 \equiv 0$ must be avoided),
...
\subsection{General Pattern: The $k$-Tuple Lattice Hierarchy}

For a prime $k$-tuple $\mathcal{H} = (0, h_2, \ldots, h_k)$ with distinct offsets modulo every prime $p \leq p_k$, the number of admissible classes at level $n$ is:
\begin{equation}
    A_{n,k} = C_k \cdot \prod_{p_k \le p \le p_n} (p-k),
\end{equation}
where $C_k$ is the base count modulo $P_{k-1}$. The per-prime thinning factor is $(p-k)$, meaning the lattice grows proportionally to $\prod_{p \le p_n} (1 - k/p)$. As $k$ increases, this product decays faster, producing increasingly sparse combinatorial spaces.

\section{Discussion: The Geometry of Variance Suppression}

In Paper 2, we observed that twin primes exhibit sub-Poisson variance across admissible classes. In this paper, we observe that prime triplets exhibit an even deeper sub-Poisson suppression. The continuous Hardy-Littlewood framework predicts Poisson variance because it models prime events as asymptotically independent. However, the discrete information geometry indicates they are not independent: they are trapped within the $\mathcal{L}_{30}$ scaffolding. 

As the constellation length $k$ increases, the primorial lattice required to support it becomes vastly more restrictive. This arithmetic restriction forces the primes to distribute more rigidly, actively suppressing local clumping (Maier-style fluctuations) and driving the Coefficient of Variation strictly below the random baseline.
...
This completes the foundational RAPF trilogy. Paper 1 established the formal necessity of discrete lattice geometries. Paper 2 empirically verified the uniform occupancy and sub-Poisson variance of these lattices for twin primes. This work extends the architecture to higher-order constellations, demonstrating that as combinatorial density thins, arithmetic rigidity increasingly dominates the statistical distribution of the primes.
